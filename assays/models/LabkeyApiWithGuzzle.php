...
class LabkeyApiWithGuzzle {
    ...
    public function getPeptideResponseCurveImage(
        ...
        if($returnObj->error == false) {
            ...
            $result = $this->downloadResponseCurvePlotImage(
                $returnObj->response
                , 'GET'
                , $peptide_sequence
                , $laboratory_abbreviation
    ...
    */
    ...
    public function resetPeptideResponseCurveImage(
        $panorama_authentication_cookie = false
        , $peptide_sequence = false
        , $laboratory_abbreviation = false
        , $celllysate_path = false
    ...
    public function resetPeptideResponseCurveAnalysis(
        $panorama_authentication_cookie = false
        , $peptide_sequence = false
        , $laboratory_abbreviation = false
        , $celllysate_path = false
...
//      $log_result_fail = json_decode($result);
    ...
    private function downloadCsv($result, $httpMethod, $parameters, $panorama_authentication_cookie) {
        ...
        if (stristr($returned_data_array["html"], 'Error executing command') == false) {
            ...
            if (!empty($matches)) {
                ...
                write_log($this->import_log_id, "Data Saved: " . $parameters['reportName']);
            }
    ...
    private function downloadResponseCurvePlotImage(
        $result
        , $httpMethod
        , $peptide_sequence
        , $laboratory_abbreviation
        ...
        if ((stristr($returned_data_array["html"], 'Error executing command') == false) && !empty($matches)) {
            ...
            write_log($this->import_log_id, "Image Saved: " . $report_name . ' - ' . $result);

        } else {
            // Return the error so it can be logged into the database.
            $result = $returned_data_array["html"];
            write_log($this->import_log_id, '<span class="import-error">Image Error</span>: ' . $report_name . ' - ' . $result);
        }
    ...
    private function downloadPlotImage($result, $httpMethod, $peptide_sequence, $laboratory_abbreviation, $report_name, $panorama_authentication_cookie) {
        $returned_data_array = json_decode($result, true);
        ...
        if (stristr($returned_data_array["html"], 'Error executing command') == false) {
            ...
            if (!empty($matches)) {
                ...
                $filename_validation_samples = $peptide_sequence . "_" . $report_name . "_" . $laboratory_abbreviation;
                ...
                $panorama_images_storage_path = $panorama_images_storage_folder .
                    "/" . $filename_validation_samples . '.png';
                $panorama_image_path = $this->panorama_images_path .
                    "/" . $report_name . '_images' .
                    "/" . $filename_validation_samples . '.png';
                file_put_contents($panorama_images_storage_path, $result);
    ...
    private function downloadChromatogramImages($result, $chromatogram_type, $parameters, $peptide_sequence, $laboratory_abbreviation) {
        ...
        write_log($this->import_log_id, "Chromatogram Image Saved: " . $result);
    }
