          ...
          ,"fnRowCallback":function(nRow, aData, iDisplayIndex){
            ...
            $(table_datas).each(function(index, single) {
              ...
              if(aData['approval_status'] == 0) {
                $(this).next('td:nth-child(2)').append('<p><i class="icon-eye-open"></i> <a href="/CPTAC-'+this_record_id+'" title="View on the portal" target="_blank">View on the Portal</a></p>');
      ...
      jQuery.ajax({
        ...
        ,success: function(ajax_return){
          ...
          if( JSON.parse(ajax_return).length > 0 ) {
            ...
            // Loop through each result and create an unordered list
            $( JSON.parse(ajax_return) ).each(function(index, single) {
              var option = $('<option value="'+single.import_log_id+'">'+single.import_date+'</option>');
