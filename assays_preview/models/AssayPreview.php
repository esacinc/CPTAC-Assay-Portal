...
namespace assays_preview\models;
...
class AssayPreview {
    ...
    public function browse_assays_public(
        $sort_field = false
        , $sort_order = 'DESC'
        , $start_record = 0
        , $stop_record = 20
                ...
                if (!empty($single_sidebar_filter)) {
                    switch ($sidebar_key) {
                        case "import_set_filter":
                            ...
                            break;
                        case "submission_filter":
                            $placeholder = array();
                            foreach ($single_sidebar_filter as $single_filter_value) {
                                $placeholder[] = "?";
                                $pdo_params[] = (int)$single_filter_value;
                            }
                            $side_bar_filter_where_array["submission_filter"] = " AND import_log.import_log_id IN (" . implode(",", $placeholder) . ") ";
                            break;
    ...
    public function get_user_associated_submission($account_id = false, $group_id = false) {
        ...
        if ($account_id) {
	      ...
	      SELECT
            import_log.import_log_id
            ,import_log.submission_id
 	      FROM import_log
	  	  JOIN user_account_import ON import_log.import_log_id = user_account_import.import_log_id
	  	  WHERE user_account_import.account_id = :account_id
	      AND import_log.group_id = :group_id");
