...
class Assay
{
      ...
      ,$side_bar_filter = array())
  {
    ...
    //$side_bar_filter_where_array["import_set_filter"] = " AND import_log.import_log_id IN (46) ";
    ...
    $sql = "SELECT SQL_CALC_FOUND_ROWS
      ...
      {$search_sql}
      {$side_bar_filter_where_sql}
      AND protein.approval_status in (0,2,4)
  ...
  public function get_user_associated_laboratories( $account_id = false ) {
  ...
  public function get_user_associated_imports( $account_id = false, $laboratory_id = false ) {
  	...
  	if($account_id) {
	      ...
	      FROM import_log
	  	  JOIN user_account_import ON import_log.import_log_id = user_account_import.import_log_id
	  	  JOIN assay_parameters_new on import_log.import_log_id = assay_parameters_new.import_log_id
	  	  WHERE user_account_import.account_id = :account_id
  ...
  public function get_public_user_associated_imports( $account_id = false ) {
  ...
  public function get_account_type($account_id){
  	...
  	$statement->bindValue(":account_id", $account_id, PDO::PARAM_STR);
  	$statement->execute();
  ...
  public function get_import_logs_by_lab_id( $laboratory_id = false ) {
    $statement = $this->db->prepare("
      ...
      FROM import_log
      JOIN assay_parameters_new on import_log.import_log_id = assay_parameters_new.import_log_id
      WHERE import_log.laboratory_id = :laboratory_id
      ");
    $statement->bindValue(":laboratory_id", $laboratory_id, PDO::PARAM_INT);
    $statement->execute();
    return $statement->fetchAll(PDO::FETCH_ASSOC);
  ...
  public function get_import_logs_by_user_id( $user_id = false ) {
    $statement = $this->db->prepare("
      SELECT
        import_log.import_log_id
        ,import_log.laboratory_id
      ...
      FROM import_log
      JOIN assay_parameters_new on import_log.import_log_id = assay_parameters_new.import_log_id
      WHERE import_log.imported_by_user_id = :user_id
      ");
    ...
    if( $import_set_id ) {
      $statement = $this->db->prepare("
        SELECT
          ...
          , approval_moderation_notes.created_date
        FROM approval_moderation_notes
        LEFT JOIN account ON approval_moderation_notes.created_by_user_id = account.account_id
