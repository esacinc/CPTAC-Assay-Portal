<?php
function datatables_browse_categories(){
	...
	$sortable_key_fields = array_keys($final_global_template_vars["categories_browse_fields"]);

	$data = $support->browse_support_categories($sortable_key_fields[$app->request()->post('iSortCol_0')], $app->request()->post('sSortDir_0')
	  , $app->request()->post('iDisplayStart'), $app->request()->post('iDisplayLength'), $app->request()->post('sSearch')
	  , $column_filters, $final_global_template_vars["categories_browse_fields"]);
