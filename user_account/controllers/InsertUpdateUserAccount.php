...
class InsertUpdateUserAccount extends Controller {

function insert_update_user_account(Request $request, Response $response, $args = []){
	//$app = \Slim\Slim::getInstance();
	global $final_global_template_vars;
	//require_once $_SERVER["PATH_TO_CORE"] . "slim_framework/GUMP/gump.class.php";
	//url parameters matched in the route
	//$params = $route->getParams();
	$account_id = isset($args["account_id"]) ? $args["account_id"] : false;
	...
	*/
	if($request->isPost()){
		//$this->container->get('logger')->info($post['group_data']);
