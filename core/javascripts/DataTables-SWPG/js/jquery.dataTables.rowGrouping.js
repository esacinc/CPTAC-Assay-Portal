        ...
        return this.each(function (index, elem) {
            ...
            function fnCreateGroupRow(sGroupCleaned, sGroup, iColspan) {
                ...
                var oGroup = {
                    ...
                    text: sGroup,
                    level: 0,
                    groupItemClass: ".group-item-" + sGroupCleaned.replace('span', 's-pan'),
                ...
                if (properties.bSetGroupingClassOnTR) {
                    nGroup.className = properties.sGroupingClass + " " + sGroupCleaned.replace('span', 's-pan');
                } else {
                    nCell.className = properties.sGroupingClass + " " + sGroupCleaned.replace('span', 's-pan');
                }

                nCell.colSpan = iColspan;
                nCell.innerHTML = properties.sGroupLabelPrefix + " " + properties.fnGroupLabelFormat
                    (sGroup == "" ? properties.sEmptyGroupLabel : sGroup, oGroup);
            ...
            function fnExpandGroup(sGroup) {
                ...
                if (properties.oHideEffect != null)
                    $(".group-item-" + sGroup.replace('span', 's-pan'), oTable)
                        [properties.oShowEffect.method](properties.oShowEffect.duration,
                        properties.oShowEffect.easing,
                        function () {
                        });
                else
                    $(".group-item-" + sGroup.replace('span', 's-pan'), oTable).show();
            ...
            function fnCollapseGroup(sGroup) {
                ...
                $('.group-item-' + sGroup.replace('span', 's-pan')).each(function () {
                ...
                if (properties.oHideEffect != null)
                    ...
                    $(".group-item-" + sGroup.replace('span', 's-pan'), oTable).hide();
...
//                          sGroupData = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]._aData[oSettings.aoColumns[properties.iGroupingColumnIndex].mDataProp];
                        ...
                        $(nTrs[i]).addClass("group-item-" + sGroupCleaned.replace('span', 's-pan'));
                        if (properties.bExpandableGrouping) {
